...
const AdminSignup = () => {
  ...
  // Check if this is first admin on mount
  useEffect(() => {
    checkFirstAdmin();
  }, []);
  ...
  const checkFirstAdmin = async () => {
    try {
      const response = await fetch('/api/auth/check-first-admin');
      const data = await response.json();
      if (data.success) {
        setIsFirstAdmin(data.data.isFirstAdmin);
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add invite code if not first admin
      if (!isFirstAdmin && inviteCode) {
  ...
  if (isFirstAdmin === null) {
    return <div style={{ padding: '40px', textAlign: 'center', color: isDark ? '#E8E4DD' : '#071A33' }}>Loading...</div>;
  }
    ...
    <div style={{
      ...
      <div className="signup-container">
        ...
        <p className="signup-subtitle">
          {isFirstAdmin ? 'Set up the first administrator account' : 'Join the SYDLINES team'}
        </p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {!isFirstAdmin && (
            <div className="invite-notice">
              ⚠️ You need an invite code to create an account. Ask your administrator.
            </div>
          )}
