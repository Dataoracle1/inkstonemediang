...
import { calculateReadingTime, formatDate } from '../utils/articleUtils';
  ...
  return (
    <>
      <SEO
        ...
        url={shareUrl}
        article={true}
        author={post.author?.name || 'SYDLINES'}
              ...
              <article className="ink-article-card ink-card" style={{ padding: 32 }}>
                ...
                <div className="ink-meta-bar" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 0', borderTop: '1px solid var(--ink-rule)', borderBottom: '1px solid var(--ink-rule)', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
                  <div className="ink-mono" style={{ display: 'flex', gap: 16, fontSize: 12, color: 'var(--ink-ink-soft)', flexWrap: 'wrap' }}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Calendar size={14} />{formatDate(post.createdAt)}</span>
                    <span>•</span>
                ...
                <div style={{ marginTop: 28, paddingTop: 24, borderTop: '1px solid var(--ink-rule)' }}>
                  ...
                  <div style={{ marginTop: 24, paddingTop: 16, borderTop: '1px solid var(--ink-rule)' }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                      <button className="ink-share-btn" onClick={() => window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`, '_blank')}>Facebook</button>
                      <button className="ink-share-btn" onClick={() => window.open(`https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(post.title)}`, '_blank')}>Twitter</button>
                      <button className="ink-share-btn" onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(post.title + ' ' + shareUrl)}`, '_blank')}>WhatsApp</button>
